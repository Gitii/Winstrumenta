...
namespace WindowExtensions
{
    public class DesktopWindow : Window
    {
        ...
        private delegate IntPtr WinProc(
            IntPtr hWnd,
            PInvoke.User32.WindowMessage Msg,
            IntPtr wParam,
            IntPtr lParam
        );
        ...
        )
        {
            switch (Msg)
            {
                case PInvoke.User32.WindowMessage.WM_GETMINMAXINFO:
                    ...
                    if (MinWidth >= 0)
                        minMaxInfo.ptMinTrackSize.x = DisplayInformation.ConvertEpxToPixel(
                            hWnd,
                            MinWidth
                        );
                    if (MinHeight >= 0)
                        minMaxInfo.ptMinTrackSize.y = DisplayInformation.ConvertEpxToPixel(
                            hWnd,
                            MinHeight
                        );
                    if (MaxWidth > 0)
                        minMaxInfo.ptMaxTrackSize.x = DisplayInformation.ConvertEpxToPixel(
                            hWnd,
                            MaxWidth
                        );
                    if (MaxHeight > 0)
                        minMaxInfo.ptMaxTrackSize.y = DisplayInformation.ConvertEpxToPixel(
                            hWnd,
                            MaxHeight
                        );
        ...
        )
        {
            ...
            hMonitor = PInvoke.User32.MonitorFromRect(
                ref prc,
                PInvoke.User32.MonitorOptions.MONITOR_DEFAULTTONEAREST
            );
