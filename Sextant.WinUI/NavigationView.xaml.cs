...
public partial class NavigationView : IView, IEnableLogger
{
    ...
    public static readonly DependencyProperty ShowDefaultBackButtonProperty =
        DependencyProperty.Register(
    ...
    )
    {
        ...
        this.WhenAnyValue(
                (x) => x.ShowDefaultBackButton,
                (x) => x.IsBackButtonVisible,
                (x, y) => x && y
            )
            .Subscribe(
                (isVisible) =>
    ...
    /// <summary>
    /// Gets or sets a value indicating whether the default back button is used/shown.
    /// </summary>
    public bool ShowDefaultBackButton
    {
        get => (bool)GetValue(ShowDefaultBackButtonProperty);
        set => SetValue(ShowDefaultBackButtonProperty, value);
    }
    ...
    /// </summary>
    public IObservable<Unit> BackRequested =>
        Observable.Merge(
            Observable
            ...
            .SelectMany(
                pageType =>
                {
                    if (resetStack)
                    {
                        ...
                        {
                            var contentTypeName =
                                mainFrame.Content?.GetType().ToString() ?? "Unknown Type";
                            _logger.Debug(
